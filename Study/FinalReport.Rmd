...
The dataset used is local to France and it include only the models sold in France. There could be some difference in countries were other models are sold.  
...
The data include the information for the VolksWagen car even if now we know that the data provided by the car maker are biaised because the company cheated on the test results.    
This study focused on the CO2 (Carbon Dioxyde). This pollutant is one of the gas that are part of the greenhouse effect.  
...
### Data Exploratory
The dataset has been downloaded from the open data French government. This dataset is about the poluants emission for the cars sold in France during the year 2015 up to October 2015. The file comes as zipped a csv file. The csv used a ";" as separator and the encoding is not UTF-8 but latin1. 
